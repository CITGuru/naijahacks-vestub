...
let getStandings = async (queryId) => {
    var result = await loadScoreAPI(`?action=get_standings&league_id=${queryId}`)
    return result
}

let getEvents = async(query) => {
    var result = await loadScoreAPI(`?action=get_events&${query}`)
